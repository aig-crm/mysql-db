...
begin declare  wherecondition varchar(500); if unit_no='' OR unit_no=null   then   set wherecondition =concat("1=1"); else   set wherecondition =concat("unit_no='",unit_no,"'"); end if; set @query1=concat( "Insert into customer_payment_plan (id, unit_no, due_date, description, percentage, net_bsp, gst, net_due, recieved) select concat(unit_no,'[',id,']') as id, unit_no, null as due_date, description, percentage, net_bsp, gst, net_due, if(rwgst>=(commulative_net_due-net_due), if(rwgst-(commulative_net_due-net_due)<=net_due, rwgst-(commulative_net_due-net_due), net_due), 0) as recieved from (select id, main.unit_no, description, percentage, net_bsp, gst, net_due, sum(net_due) over (partition by main.unit_no order by id asc) as commulative_net_due, rwgst, main.booking_date from (select id, c.unit_no->>'$.unit_no' as unit_no, description, percentage, if(nbp=0, (tbc-tbc*5/105)*percentage/100, nbp*percentage/100) as net_bsp, if(nbp=0, ((tbc-tbc*5/105)*percentage/100)*0.05, (nbp*percentage/100)*0.05) as gst, if(nbp=0, tbc*percentage/100, (nbp + (nbp*0.05))*percentage/100) as net_due, booking_date from customer c left join (select * from payment_plan)pp on c.plan->>'$.plan'=pp.plan)main left join (select unit_no, sum(rwgst) as rwgst from customer_account group by unit_no)ca on main.unit_no=ca.unit_no)final where ",wherecondition," ON DUPLICATE KEY UPDATE id=values(id), recieved=values(recieved);"); prepare stmt from @query1; execute stmt;SELECT 'Done' AS Status; deallocate prepare stmt; 
END ;;
DELIMITER ;
...
-- Dump completed on 2022-08-27 11:53:07
